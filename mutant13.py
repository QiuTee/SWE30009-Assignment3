...
def predict_ratings(user_similarity, user_item_matrix):
    ...
    #mutant 13 Change weight  
    #In this mutan I remove the absolute value of “user_similarity” before calculating the sum on each row in the predict_rating function   
    pred = mean_user_rating[:, np.newaxis] + user_similarity.dot(ratings_diff) / np.array([user_similarity.sum(axis=1)]).T
    return pred
