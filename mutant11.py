...
def predict_ratings(user_similarity, user_item_matrix):
   ...
   # 11.	Mutant 11 : Change the way the matrix is normalized
    #In this mutant I change the way of normalizing the matrix by using a different method for the “predict_rating” function 
    normalization_factor = np.sqrt(np.sum(user_similarity ** 2, axis=1))
    pred =  user_similarity.dot(ratings_diff) / normalization_factor[:, np.newaxis]
