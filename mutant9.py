...
#Mutant 9 Change column numbering by changing ` columns=range(1, user_item_matrix.shape[1] + 1)` in the pred_rating_df variable
pred_ratings_df = pd.DataFrame(pred_ratings, index=range(1, user_item_matrix.shape[0] + 1), columns=range(0, user_item_matrix.shape[1]))
