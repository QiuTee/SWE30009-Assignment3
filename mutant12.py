...
def predict_ratings(user_similarity, user_item_matrix):
    #Mutant 12 : Change the way user ratings are averaged 
    # by change the way of calculating the average user rating in the function used to predict the rating ( predict_rating ) by using “median” instead of “mean”
    median_user_rating = np.median(user_item_matrix, axis=1)
    ratings_diff = (user_item_matrix - median_user_rating[:, np.newaxis])
    pred = median_user_rating[:, np.newaxis] + user_similarity.dot(ratings_diff) / np.array([np.abs(user_similarity).sum(axis=1)]).T
    return pred
