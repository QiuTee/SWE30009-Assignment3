...
# mutant 15 : Chang the ranking prediction method by change the rating prediction method of the function “predict_rating” 
def predict_ratings(user_similarity, user_item_matrix):
    pred = user_similarity.dot(user_item_matrix) / np.array([np.abs(user_similarity).sum(axis=1)]).T
    return pred
